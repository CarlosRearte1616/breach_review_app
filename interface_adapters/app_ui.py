...
class AppUI:
    ...
    @staticmethod
    def generate_rows(
        combined_personal_info_list, documents, rows, unique_rows, is_original=False
    ):
        for doc, personal_info_list in zip(documents, combined_personal_info_list):
            for person in personal_info_list:
                if not is_original and (
                    not person.info.persons_full_name
                    or is_junk_value(person.info, person.info.persons_full_name)
                ):
                    continue
                ...
                has_other_info = any(
                    ...
                    if key != "persons_full_name" and key != "chunk_id"
                )
                ...
                if has_other_info or is_original:
                    row = {
                        ...
                        "Date of Birth": parse_date(person.info.date_of_birth)
                        if not is_original
                        else person.info.date_of_birth,
                        ...
                        "Account Number": "Yes"
                        if person.info.has_account_number
                        else "",
                        "Account PIN": "Yes"
                        if person.info.has_bank_account_pin
                        else "",
                        "Security Code": "Yes"
                        if person.info.has_credit_card_security_code
                        else "",
                        "Routing Number": "Yes"
                        if person.info.has_routing_number
                        else "",
                        "Payment Card Number": "Yes"
                        if person.info.has_payment_card_number
                        else "",
                        "Payment Card PIN": "Yes"
                        if person.info.has_payment_card_pin
                        else "",
                        "Expiration Date": "Yes"
                        if person.info.has_expiration_date
                        else "",
                        "Username with Password": "Yes"
                        if person.info.has_account_username_with_password
                        else "",
                        "Email Address with Password": "Yes"
                        if person.info.has_email_address_with_password
                        else "",
                        "Biometric Data": "Yes"
                        if person.info.has_biometric_data
                        else "",
                        "Medical Information": "Yes"
                        if person.info.has_medical_information
                        else "",
                        "Health Insurance Information": "Yes"
                        if person.info.has_health_insurance_information
                        else "",
                    }
    ...
    def display_all_inappropriate_chunks(inappropriate_chunks):
        if len(inappropriate_chunks) > 0:
            inappropriate_chunks_tab = st.expander(
                "Display all Flagged Inappropriate Chunks"
            )
    ...
    def display_flagged_documents_events(flagged_documents):
        if len(flagged_documents) > 0:
            expander = st.expander(
                f"{len(flagged_documents)} Flagged Documents for additional review"
            )
            with expander:
    ...
    def display_totals_and_elapsed_time(self, job_analytics):
        ...
        average_processing_time_per_chunk = (
            total_processing_time / job_analytics.total_chunks
        )

        self.add_to_logs(
            f"Total tokens estimated for all documents: {job_analytics.tokens_estimated}"
        )
        self.add_to_logs(
            f"Total chunks created for all documents: {job_analytics.total_chunks}"
        )
        self.add_to_logs(
            f"Average processing time per chunk: {average_processing_time_per_chunk:.2f} seconds"
        )
        cost_estimated = (
            job_analytics.tokens_estimated / 1000
        ...
        col1_2.markdown(
            f"**Avg Time per chunk:** {average_processing_time_per_chunk:.2f} seconds"
        )
        ...
        col1_3.markdown(
            f"**Most Time by Chunk:** {job_analytics.longest_chunk_processing_time:.2f} seconds"
        )
        col2_3.markdown(
            f"**Least Time by Chunk:** {job_analytics.shortest_chunk_processing_time:.2f} seconds"
        )
        col3_3.markdown(
            f"**Most Chunks Created Per Doc:** {job_analytics.most_chunks_created}"
        )
        col4_3.markdown(
            f"**Documents Not Chunked** {job_analytics.documents_not_chunked}"
        )
        col5_3.markdown(
            f"**Number of Chunks without Entities** {job_analytics.number_no_entities_found_chunks}"
        )
        col1_4, col2_4, col3_4, col4_4 = st.columns(4)
        col1_4.markdown(
            f"**Total Timed Out Chunks:** {job_analytics.timed_out_chunks_count}"
        )
        col2_4.markdown(f"**Total Failed Chunks:** {job_analytics.failed_chunks_count}")
        col3_4.markdown(
            f"**Chunks Flagged Inappropriate:** {job_analytics.chunks_flagged_inappropriate}"
        )
        col4_4.markdown(
            f"**Total Validation Errors:** {job_analytics.validation_errors}"
        )

    def complete_job(
        self, combined_personal_info_list, original_personal_info_list, flagged_events
    ):
        if len(st.session_state.documents) == 0:
            st.session_state.documents = self.documents
        # Display the DataFrame and get the Excel data
        excel_data = self.display_results(
