...
class LangchainProcessor(LLMProcessor):
    ...
    async def agenerate_extraction(self, input_text, document_name, chunk_id):
        ...
        if toggle_status == 1:
            ...
            prompt = self.prompt_creator.create_extraction_prompt().format_prompt(input=text_input).to_string()
            print("Prompt: " + prompt)
            print()
            chain = LLMChain(llm=self.llm, prompt=self.prompt_creator.create_extraction_prompt(),
                            verbose=True, tags=[f"document_name:{document_name}", f"chunk_id:{chunk_id}"])
    ...
    async def asequential_generate(self, input_text):
        llm = AzureChatOpenAI(
            ...
            max_tokens=650,
            temperature=config.TEMPERATURE)
        classification_chain = LLMChain(llm=llm, prompt=self.prompt_creator.create_classification_prompt(),
                                        verbose=True)
        ...
        format_instructions = ClassifiedInstructor(
            pydantic_object=PersonalInfoList).get_format_instructions(contained_entities=classified_entities)

        if format_instructions is None:
            print("NO ENTITIES FOUND")
            return LLMResponse(text=json.dumps({"data": []}), finish_reason="No entities found")
        print(f"format_instructions: {format_instructions}")

        extraction_chain = LLMChain(llm=self.llm,
                                    prompt=self.prompt_creator.create_classified_extraction_prompt(format_instructions),
                                    verbose=True,
